...
for c in lista_c:
    ...
    if not fila.empty:
        ...
        if es_baja:
            ...
            if p_hist > 0.01 or abs(rb_hist) > 0.01:
                mov_asientos -= rb_hist
                mov_asientos += aa_hist
                
    else:
        # Fila empty! Let's check if it terminated natively in m_idx
        f_fin_c = pd.to_datetime(c['Fin'])
        if f_fin_c.month == m_idx and f_fin_c.year == a:
            rb_hist, aa_hist, p_hist = simular_libro_mayor(c, tab, f_act, rems_grupos.get(c['Codigo_Interno'], []), tc_ini_hist, vp, rou, ignore_baja=True)
            if p_hist > 0.01 or abs(rb_hist) > 0.01:
    ...
    if abs(diff) > 1.0:
        diffs.append((c['Codigo_Interno'], mov_recon, mov_asientos, diff))

print('Total Diff:', sum(d[3] for d in diffs))
for d in diffs:
    print(d)
