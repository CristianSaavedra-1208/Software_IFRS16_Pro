...
print("Contratos con diferencias > 1000 pesos en Q1 2026:")
for c in lista_c_todas:
    ...
    if abs(diff_rou) > 1000 or abs(diff_aa) > 1000:
        print(f"[{c_code}] Diff ROU: {diff_rou:,.0f} | Diff AA: {diff_aa:,.0f} | Mov Recon ROU: {mov_recon_rou:,.0f} | Asiento ROU: {asiento_rou:,.0f} | Estado: {c['Estado']}")
