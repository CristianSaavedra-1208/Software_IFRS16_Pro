...
f_ant = pd.to_datetime(date(a, m_idx - 1, 1)) + relativedelta(day=31) # End of Jan 2026

diff_total = 0

print("Verificando Pasivo para Febrero 2026...")
...
for c in lista_c:
    ...
    if not past_dic.empty and not es_baja_dic:
        ...
        if f_ini.year == a and f_ini.month == m_idx: # Reconocimiento ocurre en el mes, saldo anterior es 0
            recon_pasivo_dic = 0
    ...
    if f_ini.month == m_idx and f_ini.year == a:
        ...
        # Wait, if it was terminated in Jan, the actual last target balance in Jan was 0!
        # Because the accounting system forced it to 0 in Jan!
        if es_baja_dic:
